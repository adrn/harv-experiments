"""Shared helpers for the reductions.

The important one is :func:`as_periodogram_result`: ``PeriodogramResult`` is a plain
``eqx.Module``, so kepmodel's ``z0`` and the Keplerian reference ``R`` can be wrapped
in one and fed through harv's own ``tempered_period_prior`` / ``peak_period_prior``.
That is what makes the prior-quality and calibration comparisons apples-to-apples
with no duplicated prior code.
"""
...
def as_periodogram_result(
    ...
    *,
    ...
    ``frequency`` is in ``1 / day`` and ``t_span`` in days. Used for kepmodel's
    ``z0`` and the reference ``R`` so they can drive harv's prior builders unchanged.
    """
...
def ln_period_density(
    ...
    ``tempered_period_prior`` returned). ``log_prob_ln`` is already the density per
    unit ln-period and is unit-invariant, but it is only normalized over the prior's
    own support, so renormalize on the evaluation grid to make the two curves
    comparable.
    """
...
def tv_distance(
    ...
    ``0`` means identical, ``1`` means disjoint support. Chosen over a distance
    between raw nats curves because the frequency-independent part of the Occam
    factor shifts ``Delta`` bodily without changing any inference, and a raw-curve
    metric would score that shift as error.
    """
    return 0.5 * float(np.trapezoid(np.abs(rho_a - rho_b), ln_p_grid))


def detection_scalars(path: object) -> tuple[dict, dict]:
    """``(by_sim, attrs)``: each sim's grid-maximum per statistic, plus its attrs.

    Shared by :mod:`kepcmp.reduce.roc` and :mod:`kepcmp.reduce.regime_map` so the
    detection scalar is defined in exactly one place. ``attrs`` carries the
    ``z0_usable`` flag alongside the cell axes, so callers can drop the sims where
    kepmodel's ``z0`` is a singular-matrix artifact rather than a measurement.
    """
    from kepcmp.artifact import ArtifactReader

    by_sim: dict[str, dict[str, float]] = {}
    attrs: dict[str, dict] = {}
    with ArtifactReader(path) as art:  # type: ignore[arg-type]
        for sim_id in art.sim_ids():
            a = dict(art.attrs(sim_id))
            a["z0_usable"] = art.z0_usable(sim_id)
            attrs[sim_id] = a
            scal = {"z0": float(np.max(art.z0(sim_id)))}
            for key in art.stat_keys(sim_id):
                scal[f"delta:{key}"] = float(np.max(art.stat(sim_id, key)["delta"]))
            by_sim[sim_id] = scal
    return by_sim, attrs
