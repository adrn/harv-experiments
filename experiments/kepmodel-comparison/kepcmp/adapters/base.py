...
class MargBlocks(NamedTuple):
    ...
    :mod:`kepcmp.linalg` decomposes harv's own matrices rather than a
...
class Adapter(Protocol):
    ...
    2 for RV (``cos``, ``sin``), 4 for Gaia astrometry (the scan-angle-projected
    pair), which is why ``n_terms=1`` lines up with kepmodel's ``d`` in both cases.
    ``n_base_columns`` is ``p``: 1 for RV (the offset), 5 for Gaia (the astrometric
    solution). Together they set where the profile statistic stops existing.
    """
    ...
    def amplitude(self, cell: Cell) -> Q:
        """The injected signal amplitude for ``cell``: ``snr * sigma_n``."""
        ...
    ...
    def data_rms(self, data: AbstractData) -> Q:
        """RMS of the observations about their mean --- what ``sigma_amp`` scales."""
        ...
    ...
    def data_arrays(self, data: AbstractData) -> dict[str, NDArray[np.float64]]:
        """Per-epoch arrays to store, so ``prior_quality`` need not re-simulate."""
        ...
    ...
    def trial_prior(
        self, data: AbstractData, *, n_terms: int, sigma_amp: Any = None
    ) -> HarvPrior:
    ...
    def marg_blocks_batched(
        ...
        *,
        ...
        """Design matrices at every period, plus the period-independent pieces."""
        ...
    ...
    def kepmodel_model(self, data: AbstractData) -> Any:
        """A kepmodel model with base linear columns installed and covariance set."""
        ...
    ...
    def science_prior(self, data: AbstractData, period_prior: Any) -> HarvPrior:
        """Full Keplerian prior for the downstream rejection run."""
        ...

    def science_model(self) -> Any:
        """The harv component model for the downstream rejection run."""
        ...
