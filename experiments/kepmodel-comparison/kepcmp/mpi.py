...
:mod:`kepcmp.merge` combines the per-rank files afterwards. MPI is a launcher, not a
message bus.
...
def mpi_context(force: bool = False) -> tuple[Any, int, int]:
    ...
    **The import is guarded, and that is not fussiness.** ``import mpi4py.MPI`` runs
    ...
    unconditionally would make a plain ``python -m kepcmp.run``, and every test that
