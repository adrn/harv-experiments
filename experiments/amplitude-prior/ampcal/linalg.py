...
gives ``Delta = z0 - Occam - Shrinkage`` with ``z0`` the profile (least-squares) log
ratio. ``Occam`` is what a period-dependent amplitude prior tilts, and that tilt is
the mechanism this experiment measures --- so the decomposition here is a result, not
only a diagnostic.
...
class DeltaDecomposition(NamedTuple):
    ...
    z0: float
    """``1/2 (chi2_H - chi2_K)``, the profile (least-squares) log-likelihood ratio."""
...
#
# :func:`evaluate` is the reference implementation and is what the correctness gate
# in :mod:`ampcal.identity` validates against harv's own ``log_prob``. It is also a
# python-level call per frequency, which is far too slow for the production grid
# (~6700 sims x 24 arms x ~1800 frequencies). :func:`evaluate_batched` is the
# vectorized equivalent; ``tests/test_linalg.py`` pins it to :func:`evaluate` --- for
# period-dependent priors as well as flat ones --- so the fast path can never silently
...
def evaluate_batched(
    ...
    prior_mu, prior_scale_tril
        Prior mean and Cholesky factor. Either shared across frequencies --- ``(k,)``
        and ``(k, k)`` --- or per-frequency, ``(n_freq, k)`` and ``(n_freq, k, k)``.
    ...
    **Per-frequency priors are not an optional extra here.** A period-dependent
    amplitude prior is the hypothesis this experiment tests, and its whole mechanism is
    that ``occam(nu)`` picks up the prior's tilt. Broadcasting one shared ``Lambda``
    across the grid would silently report the flat arm's decomposition for every arm.
    The cost is a per-frequency ``slogdet`` and ``solve`` instead of one shared --- both
    of which were already per-frequency, since ``M = I + B^T B`` depends on ``X``.
    """
    import jax.numpy as jnp

    n_freq, n, k = X.shape
    mu = jnp.broadcast_to(jnp.asarray(prior_mu), (n_freq, k))
    chol = jnp.broadcast_to(jnp.asarray(prior_scale_tril), (n_freq, k, k))

    inv_sigma = 1.0 / jnp.sqrt(cov)
    xw = X * inv_sigma[None, :, None]
    yw = (y[None, :] - jnp.einsum("fnk,fk->fn", X, mu)) * inv_sigma[None, :]
