...
The grid never swept these -- ``ArmRecord`` carries the amplitude scale and nothing for
the offset scales -- so this section measures them directly, and cheaply, by re-running
...
from ampcal.grid import Cell, shared_frequency_grid
...
def _rv_sweep(data: Any, n_sims: int) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    cell = Cell(
        population="independent", period_ratio=0.3, snr=10.0, eccentricity=0.0,
        n_obs=32,
    )
    rows = []
